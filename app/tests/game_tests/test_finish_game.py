...
from app.routers.games.services import finish_game
from app.routers.players.utils import find_player_by_id
from app.routers.players.schemas import PlayerRol
...
def test_finish_game_successfully():
    cleanup_database()
    players = []
    ...
    with db_session:
        game = select(g for g in Game if g.name == "TestGame").first()
        for p in game.players:
            if p.rol == PlayerRol.THE_THING:
                p.rol = PlayerRol.ELIMINATED
    ...
    response = client.post(f"/games/TestGame/play-action-card", json=game_data)
    response.status_code == 200, "El codigo de estado de la respuesta no es 200 (OK)"
    cleanup_database()
