...
def test_cancel_game_successfully():
    cleanup_database()
    players = []
...
def test_cancel_inexistent_game():
    ...
    for player in players[1:]:
        ...
        client.patch("/games/join/TestGame", json=game_data)
    response = client.delete(
        f"/games/cancel/BadGameName?player_id={players[0].id}")
    assert response.status_code == 404, "El codigo de estado de la respuesta no es 404 (Not Found)"
    cleanup_database()


def test_cancel_game_not_in_unstarted_state():
    cleanup_database()
    players = []
    names = ["Ignacio", "Anelio", "Ezequiel", "Amparo"]
    for name in names:
    ...
    for player in players[1:]:
        ...
        client.patch("/games/join/TestGame", json=game_data)
    # This change to STARTED status
    client.patch(f"/games/TestGame/init?host_player_id={players[0].id}")
    response = client.delete(
        f"/games/cancel/TestGame?player_id={players[0].id}")
    assert response.status_code == 400, f"El codigo de estado de la respuesta no es 400 (Bad Request)"
    cleanup_database()


def test_cancel_game_bad_host_id():
    cleanup_database()
    players = []
    names = ["Ignacio", "Anelio", "Ezequiel", "Amparo"]
    for name in names:
    ...
    # This change to STARTED status
    response = client.delete(
        f"/games/cancel/TestGame?player_id={players[1].id}")
    assert response.status_code == 400, f"El codigo de estado de la respuesta no es 400 (Bad Request)"
    cleanup_database()
