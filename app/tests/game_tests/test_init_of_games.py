...
def test_init_game_successfully():
    cleanup_database()
    players = []
    ...
    response = client.patch(
        f"/games/TestGame/init?host_player_id={players[0].id}")
    assert response.status_code == 200, "El codigo de estado de la respuesta no coincide con el esperado"
    cleanup_database()


def test_init_game_less_min_players():
    cleanup_database()
    players = []
    names = ["Ignacio", "Anelio", "Ezequiel",
             "Amparo", "Santiago", "Nehuen", "Gabriel"]
...
def test_init_game_bad_host_id():
    cleanup_database()
    players = []
    names = ["Ignacio", "Anelio", "Ezequiel",
             "Amparo", "Santiago", "Nehuen", "Gabriel"]
    ...
    for player in players[1:]:
        ...
        client.patch("/games/join/TestGame", json=game_data)
    response = client.patch(
        f"/games/TestGame/init?host_player_id={players[0].id + 1}")
    assert response.status_code == 400, "El codigo de estado de la respuesta no es 400 (Bad Request)"
    cleanup_database()
